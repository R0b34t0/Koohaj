<template>
  <ul v-if="this.searchVal" class="listStyle">
    <li class="popisJela" v-for="(jelo, index) in this.mealsDb" :key="jelo[0]">
      <p :class="[isClicked ? 'clickedTitle' : 'unClickedTitle']">
        {{ jelo[1].imeJela }}
      </p>
      <div :class="[isClicked ? 'clickedMeal' : 'unClickedMeal']">
        ...
        <img
          class="images"
          v-on="getImage(jelo[1].imeSlike)"
          @click="openMeal(index)"
          :src="jelo[1].link"
        />
...
export default {
  ...
  beforeUpdate() {
    let tempArr = [];
    tempArr = this.mealsDb.filter((value) =>
      value[1].imeJela.toLowerCase().startsWith(this.searchVal)
    );
